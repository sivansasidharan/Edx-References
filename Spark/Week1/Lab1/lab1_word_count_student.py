...
# TODO: Replace <FILL IN> with appropriate code
from operator import add
totalCount = wordCounts.map(lambda (k, v): (v)).reduce(lambda a, b: a + b)
...
# TODO: Replace <FILL IN> with appropriate code
def wordCount(wordListRDD): return wordListRDD.map(lambda x: (x, 1)).reduceByKey(lambda x, y: x + y)
...
import re
import string
def removePunctuation(s):return re.sub('[%s]' % re.escape(string.punctuation), '', s).strip().lower()
